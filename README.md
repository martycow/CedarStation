...
<code>var logger = new CedarLogger(LoggerSettings);  <-- I consider LoggerSettings as a top-level asset dependency, so I drag-n-drop it on ApplicationScope object on Application scene</code>
  
<code>logger.Line();  <-- Just outputs a line filled with a specific symbols and having a specific string length. </code>
  
<code>logger.Info(SystemTag, string);  <-- General log message. Adds [SystemTag] in front of the message; </code>

<code>logger.Warn(SystemTag, string);  <-- Same but for warnings. I barely use them </code>
  
<code>logger.Warn(SystemTag, string);  <-- Same but for errors. </code>
  
<code>logger.Success(SystemTag, string); <-- Log message which tells you that something was done right. Adds [Success] [SystemTag] in front of the message. Always green </code>
  
<code>logger.Fail(SystemTag, string) <-- Same but if everything went not as expected. Adds [Fail] [SystemTag] in front of the message. Always red </code>
